...
class KinovaRobotControllerROS2(Node):
    ...
    async def get_current_pose(self):
        ...
        try:
            # Create a one-time subscription to get the current pose
            pose_received = asyncio.Event()
            current_pose = None
            ...
            # Subscribe to the cartesian state topic
            subscription = self.create_subscription(
                PoseStamped,
                '/my_gen3/robot_cartesian_state',
                pose_callback,
                1  # QoS depth of 1 to get the most recent message
            )
            
            # Wait for a message with timeout
            try:
                await asyncio.wait_for(pose_received.wait(), timeout=5.0)
            except asyncio.TimeoutError:
                self.get_logger().warn('Timeout waiting for current pose')
                return None
            finally:
                # Clean up the subscription
                self.destroy_subscription(subscription)
            
            self.get_logger().info(f'Current pose retrieved: position=({current_pose.position.x:.3f}, {current_pose.position.y:.3f}, {current_pose.position.z:.3f})')
            return current_pose
