...
class FaceDetectionNode(Node):
    def __init__(self):
        ...
        # Control subscribers
        self.start_face_detection_sub = self.create_subscription(
            Bool, '/start_face_detection', self.start_face_detection_callback, 10)
        self.stop_face_detection_sub = self.create_subscription(
            Bool, '/stop_face_detection', self.stop_face_detection_callback, 10)
        
        # Publishers - using same topics as food detection for servoing compatibility
        self.position_vector_pub = self.create_publisher(Vector3, '/position_vector', 10)
        self.face_detection_complete_pub = self.create_publisher(Bool, '/face_detection_complete', 10)
        
        # RViz visualization publishers (same as original face detection script)
    ...
    def start_face_detection_callback(self, msg):
        if msg.data:
            self.get_logger().info('Starting face detection...')
            self.face_detection_active = True
    
    def stop_face_detection_callback(self, msg):
        if msg.data:
    ...
    def publish_visualization_markers(self, mouth_center_x, mouth_center_y, depth_value):
        ...
        if mouth_in_effector and finger_midpoint:
            ...
            # Check if we're close enough (within 3cm)
            if np.linalg.norm([vector.x, vector.y, vector.z]) < 0.03:
                self.get_logger().info("Robot is within 3cm of mouth, goal reached")
    ...
    def draw_mouth_landmarks_on_image(self, rgb_image, detection_result):
        ...
        for idx in range(len(face_landmarks_list)):
            ...
            # identify all of the lip landmarks
            ...
            # Publish position vector for servoing
    ...
    def process_frame(self):
        ...
        if not self.face_detection_active or self.latest_color_image is None or self.detector is None:
            return
        ...
        try:
            ...
            # Draw mouth landmarks and process
            annotated_image = self.draw_mouth_landmarks_on_image(rgb_frame, detection_result)

            # Display the annotated image
            cv2.imshow('Face Detection - Mouth Landmarks', cv2.cvtColor(annotated_image, cv2.COLOR_RGB2BGR))
