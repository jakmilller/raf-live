...
class CoordinateTransforms:
    ...
    def calculate_position_vector_from_pixel(self, pixel_x, pixel_y, depth_method='single'):
        """
        Calculate position vector from finger midpoint to target pixel
        
        Args:
            pixel_x: Target pixel x coordinate
            pixel_y: Target pixel y coordinate  
            depth_method: 'single' for single pixel depth, 'mask' for mask average depth
        ...
        try:
            ...
            # Get depth value
            if depth_method == 'single':
                depth_mm = depth_image[int(pixel_y), int(pixel_x)]
                avg_depth = depth_mm / 1000.0  # Convert mm to m
            else:
                # This would be used for mask-based depth calculation
                # For now, default to single pixel
                depth_mm = depth_image[int(pixel_y), int(pixel_x)]
                avg_depth = depth_mm / 1000.0
    ...
    def calculate_position_vector_from_mask(self, pixel_x, pixel_y, segmentation_mask):
        """
